...
{
	/// <summary>
	/// Description of ParseRackTiming.
	/// </summary>
	public partial class ReceptParserBlueII
	{
		
		private bool LoadRackTiming(TreeNode nd, TreeView tr, XmlNode node, bool pridanaUrovenLoad)
		{	eaColorID=0;	// easy cooking color
 		...
 		nd.SelectedImageIndex=3;
				...
				{	var ps=new ReceptStruct.ProgStruct();
		   			ps.t_typeOfProgram=ReceptStruct.t_TypeOfProgram.t_RackTiming;	// typ programu
		...
		private void SaveParseRackTiming(XmlWriter w, TreeNode tn, ReceptStruct.ProgStruct ps )
		{
			w.WriteStartElement("RackTiming");		//ex element
			...
			if(ps!=null)
			{	// ******************* kroky racktimingu
				if(ps.RackTimingTimerSteps!=null)
				{	foreach(ReceptStruct.RackTimingTimer st in ps.RackTimingTimerSteps)
					{	w.WriteStartElement("Timer");	//element step
						w.WriteAttributeString("name",st.Name);
						
						w.WriteStartElement("EndBy");		w.WriteAttributeString("v",((int)st.EndBy).ToString()); w.WriteEndElement();
						
						if(st.EndBy== ReceptStruct.RackTimingEndBy.seByTime)	// ukonceno casem
						{	w.WriteStartElement("Time");		w.WriteAttributeString("v",((int)st.Time).ToString());  w.WriteEndElement();
						}
						if(st.EndBy== ReceptStruct.RackTimingEndBy.seByCoreProbe)	// ukonceno teplotou jehly
						{	w.WriteStartElement("CoreT");		w.WriteAttributeString("v",((int)st.CoreT).ToString());  w.WriteEndElement();
						}
													
						
						w.WriteEndElement();	// element rack timing konec
					}
				}
